...
| Дата | Изменения |
| ----------- | --------- |
| 2026/05 |  - добавлены алгоритмы фильтрации вибраций подстраивающимися режекторными фильтрами по результатам работы FFT на борту (FFT вычисляет три частоты для каждой оси, формируются три режекторных фильтра на этих частотах и сверху накладывается LPF) <br/>  - добавлена задача RC_emulation, отправляющая в ту же очередь что и пульт команды управления  <br/>  - добавлены наброски для вывода телеметрии по MSP  <br/>  - добавлен ряд функций для ICM45686 <br/>  - в расчет ПИДов добавлена FF составляющая, D переделана под (pos-prev_pos) вместо (error-prev_error)   |
| 2026/04 |  - код вычесан под ESP-IDF 6.0 <br/>  - компоненты разложены по папкам (для возможности расширения перечня компонентов на будущее)  <br/>  добавлен алгоритм вычисления углов по VQF  <br/>  - добавлена возможность выбора алгоритма вычисления углов (Fusion/VQF) и управления двигателями (PWM/Dshot) <br/>  - добавлена задача FFT, которая в реальном времени вычисляет доминирующую частоту вибраций и передает в main_flying_cycle для последующего динамического расчета коэффициентов режекторных фильтров (через векторные инструкции библиотеки esp-dsp)  | 
| 2026/03 |  - код вычесан под ESP-IDF 5.5.3, внесены изменения в cmake <br/>  - добавлен обновленный фильтр Маджвика (https://github.com/xioTechnologies/Fusion), который, как выяснилось, работает значительно быстрее оригинального (старый пока доступен через #define USING_OLD_MADGWICK) <br/>  - добавлены наброски для поддержки Dshot (через RMT)  |
| 2026/02 |  - в конце main_flying_cycle дописан кусок, детектирующий "аварию". При фиксации в полете угла по pitch или roll, превышающего установленный предел, или при фиксации акселерометром ускорения более заданного порога в течение заданного времени аварийно отключаются двигатели и запускается задача emergency_mode <br/>  - чуть изменена запись логов, чтобы можно было внепланово записывать снимок данных в момент активации emergency_mode |
