...
//настраиваем модуль ШИМ на борту ESP32. Делаем это в начале программы, чтобы сразу выдать на ESC управляющие сигналы и они не пищали  
  ...
  configuring_channel_for_PWM(0, ENGINE_OUTPUT_0_PIN) ;
  configuring_channel_for_PWM(1, ENGINE_OUTPUT_1_PIN);
  configuring_channel_for_PWM(2, ENGINE_OUTPUT_2_PIN);
  configuring_channel_for_PWM(3, ENGINE_OUTPUT_3_PIN);
...
*/
  ...
  if (MPU6000_init(MPU6000_2, 0x06) != ESP_OK) {
    error_code = 19;
    xTaskCreate(error_code_LED_blinking,"error_code_LED_blinking",2048,(void *)&error_code,0,NULL);
    while(1) {vTaskDelay(1000/portTICK_PERIOD_MS);} 
  }
