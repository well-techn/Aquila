...
QueueHandle_t fft_to_main_queue = NULL;//очередь для передачи пиковой частоты вибрации из задачи FFT в main
...
TaskHandle_t task_handle_main_flying_cycle = NULL;
TaskHandle_t task_handle_send_data_to_RC = NULL;
TaskHandle_t task_handle_init = NULL;
TaskHandle_t task_handle_performace_measurement = NULL;
TaskHandle_t task_handle_INA219_read_and_process_data = NULL;
TaskHandle_t task_handle_mag_read_and_process_data = NULL;
TaskHandle_t task_handle_PCA9685_control = NULL;
TaskHandle_t task_handle_MCP23017_monitoring_and_control = NULL;
TaskHandle_t task_handle_writing_logs_to_flash = NULL;
TaskHandle_t task_handle_mavlink_telemetry = NULL;
TaskHandle_t task_handle_lidar_read_and_process_data = NULL;
TaskHandle_t task_handle_MS5611_read_and_process_data = NULL;
TaskHandle_t task_handle_px4flow_read_and_process_data = NULL;
...
//настраиваем модуль для управления ESC. Делаем это в начале программы, чтобы сразу выдать на ESC управляющие сигналы и они не пищали  
  ESP_LOGI(TAG_INIT,"Настройка модуля для управления двигателями.....");
  esc_control_initialize(-1);
  ESP_LOGI(TAG_INIT,"Модуль для управления двигателями настроен\n");

  if (!(MCP23017_get_inputs_state() & 0b00000001))        //если стоит DI0 - заходим в меню настойки (запускаем отдельную задачу settings)
  {
    ESP_LOGE(TAG_INIT,"Нажата кнопка на DI0, входим в режим конфигурирования.....");
#ifdef TELNET_CONF_MODE
...
//если стоит перемычка DI2 - запускаем калибровку ESC
  ...
  if (!(MCP23017_get_inputs_state() & 0b00000100))                                      
    { 
      ...
      ESP_ERROR_CHECK(ledc_update_duty(ENGINE_PWM_MODE, 0));
      ESP_ERROR_CHECK(ledc_set_duty(ENGINE_PWM_MODE, 1, ENGINE_MIN_SIGNAL +1));
      ESP_ERROR_CHECK(ledc_update_duty(ENGINE_PWM_MODE, 1));
      ESP_ERROR_CHECK(ledc_set_duty(ENGINE_PWM_MODE, 2, ENGINE_MIN_SIGNAL +1));
      ESP_ERROR_CHECK(ledc_update_duty(ENGINE_PWM_MODE, 2));
      ESP_ERROR_CHECK(ledc_set_duty(ENGINE_PWM_MODE, 3, ENGINE_MIN_SIGNAL +1));
      ESP_ERROR_CHECK(ledc_update_duty(ENGINE_PWM_MODE, 3));
...
*/  
  ...
  if (fft_to_main_queue == NULL) {
    ...
    else ESP_LOGI(TAG_INIT,"Очередь для передачи данных от FFT в main_flying_cycle успешно создана"); 
#endif
...
//проверяем напряжение аккумулятора и ток потребления системы
  ...
  if (fft_rb_handle!= NULL)  
    ...
    ESP_LOGE(TAG_INIT,"Кольцевой буфер для fft не создан\n");
    error_code = 2;
    xTaskCreate(error_code_LED_blinking,"error_code_LED_blinking",2048,(void *)&error_code,0,NULL);
    while(1) {vTaskDelay(1000/portTICK_PERIOD_MS);} 
  }

  ESP_LOGI(TAG_INIT,"Создаем задачу fft.....");
  task_handle_fft = xTaskCreateStaticPinnedToCore(fft,"fft",FFT_STACK_SIZE ,NULL,FFT_PRIORITY,fft_stack,&fft_TCB_buffer,0);
  if (task_handle_fft != NULL)  
    ESP_LOGI(TAG_INIT,"Задача fft создана на ядре 0\n");
  else {
    ESP_LOGE(TAG_INIT,"Задача для fft не создана");
    error_code = 2;
    xTaskCreate(error_code_LED_blinking,"error_code_LED_blinking",2048,(void *)&error_code,0,NULL);
    while(1) {vTaskDelay(1000/portTICK_PERIOD_MS);} 
  }
