...
void create_and_configure_dshot_rmt_enviroment(rmt_channel_handle_t *tx_channel, 
                                                      uint8_t* tx_gpio_number,
...
//включаем каналы
    for (uint8_t i = 0; i < number_of_channels; i++) ESP_ERROR_CHECK(rmt_enable(tx_channel[i]));

//создаем и инициализируем 4 энкодера    
    dshot_esc_encoder_config_t encoder_config = {
        .resolution = DSHOT_ESC_RESOLUTION_HZ,
        .baud_rate = DSHOT_BITRATE, // DSHOT300 protocol
        .post_delay_us = 2000, // extra delay between each frame
    };
    for (uint8_t i = 0; i < number_of_channels; i++) ESP_ERROR_CHECK(rmt_new_dshot_esc_encoder(&encoder_config, &dshot_encoder[i]));

// создаем и инициализируем синхромеханизм
/*        rmt_sync_manager_config_t synchro_config = {
            .tx_channel_array = tx_channel,
            .array_size = number_of_channels,
        };
