...
float avg_filter_1d(float* array, float new_data_point, uint8_t filter_length)
{
    ...
    memmove(array, array + 1, (filter_length - 1) * sizeof(float));
    array[filter_length-1] = new_data_point;
    for (uint8_t i = 0; i < filter_length; i++) accum += array[i];
...
//сдвигаем на 1 старый объем данных
    memmove(i_arr, i_arr + 1, (window_size - 1) * sizeof(float));
...
//производим сортировку и выдаем результат    
...
{   
    float omega = 2.0 * (float)M_PI * f_cut / f_sampling; // Нормированная частота
...
uint8_t arrays_are_equal (const  uint8_t* array_1, const uint8_t* array_2, uint8_t length)
{
...
uint8_t all_array_elements_are_equal(const uint8_t* array, uint8_t length) 
{
