#include "MPU6000.h"
#include "wt_spi.h" 
#include "esp_log.h"
#include <rom/ets_sys.h>
#include "math.h"
#include "wt_alldef.h"



extern char *TAG_MPU6000;;

esp_err_t MPU6000_communication_check(spi_device_handle_t MPU_handle) {

    esp_err_t err = ESP_FAIL;

    if ((SPI_read_byte (MPU_handle, 0, 0, 8, MPU6000_WHO_AM_I | MPU6000_READ_FLAG, 0)) == 0x68) 
    {
        ESP_LOGI(TAG_MPU6000,"Связь с MPU6000 установлена");
        err = ESP_OK;
    }
    else ESP_LOGE(TAG_MPU6000,"Связь с MPU6000 не установлена\n");
    
    return err;
}     

esp_err_t MPU6000_self_test(spi_device_handle_t MPU_handle) {

 uint8_t buf[4];
 int8_t factory_trim_values_accel[3];
 float factory_trim_calculated_values_accel[3];
 int8_t factory_trim_values_gyro[3];
 float factory_trim_calculated_values_gyro[3];
 uint8_t sensor_data[14];
 int16_t accel_raw[3];
 int16_t gyro_raw[3];
 int16_t accel_raw_test[3];
 int16_t gyro_raw_test[3];
 float str_accel[3];
 float str_gyro[3];
 float final_value_accel[3];
 float final_value_gyro[3];
 
 esp_err_t err = ESP_OK;

    SPI_read_bytes(MPU_handle, 0, 0, 8, MPU6000_SELF_TEST_X | MPU6000_READ_FLAG, 0, buf, 4); //reading Factory trim values from accel and gyro reristers

    factory_trim_values_accel[0] = ((buf[0] >> 3) & 0b00011100) | ((buf[3] >> 4) & 0b00000011); //assembling AX trim value
    factory_trim_values_accel[1] = ((buf[1] >> 3) & 0b00011100) | ((buf[3] >> 2) & 0b00000011); //assembling AY trim value 
    factory_trim_values_accel[2] = ((buf[2] >> 3) & 0b00011100) | (buf[3] & 0b00000011); //assembling AZ trim value

    factory_trim_values_gyro[0] = buf[0] & 0b00011111; //assembling GX trim value
    factory_trim_values_gyro[1] = buf[1] & 0b00011111; //assembling GY trim value
    factory_trim_values_gyro[2] = buf[2] & 0b00011111; //assembling GZ trim value

    ESP_LOGI(TAG_MPU6000,"Factory trim values для акселерометра %d, %d, %d",factory_trim_values_accel[0],factory_trim_values_accel[1],factory_trim_values_accel[2]);
    ESP_LOGI(TAG_MPU6000,"Factory trim values для гироскопа %d, %d, %d",factory_trim_values_gyro[0],factory_trim_values_gyro[1],factory_trim_values_gyro[2]);
    
    for (uint8_t i = 0; i < 3; i++) {
        if (factory_trim_values_accel[i] == 0) factory_trim_calculated_values_accel[i] = 0;
        else factory_trim_calculated_values_accel[i] = 4096.0 * 0.34 * powf((0.92 / 0.34),((float)(factory_trim_values_accel[i] - 1.0) / 30.0));
        
        if (factory_trim_values_gyro[i] == 0) factory_trim_calculated_values_gyro[i] = 0;
        else factory_trim_calculated_values_gyro[i] = 25.0 * 131.0 * powf(1.046,(float)(factory_trim_values_gyro[i] - 1));
    }

    factory_trim_calculated_values_gyro[1] *= -1.0;     //as Y should be -25, not 25 as above  
    
    ESP_LOGI(TAG_MPU6000,"Вычисленные значения для акселерометра %0.2f, %0.2f, %0.2f",factory_trim_calculated_values_accel[0],factory_trim_calculated_values_accel[1],factory_trim_calculated_values_accel[2]);
    ESP_LOGI(TAG_MPU6000,"Вычисленные значения для гироскопа %0.2f, %0.2f, %0.2f",factory_trim_calculated_values_gyro[0],factory_trim_calculated_values_gyro[1],factory_trim_calculated_values_gyro[2]);
    
    //settings up minimum registers to read data (accel +-8G, gyro +-250dps)
    SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_PWR_MGMT_1, 0, 0x80);
    ets_delay_us(10000); 
    SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_SIGNAL_PATH_RESET, 0, 0x07);
    ets_delay_us(10000); 
    SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_PWR_MGMT_1, 0, 0x00);
    ets_delay_us(10000); 
    SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_GYRO_CONFIG, 0, 0x00);          //+-250dps
    ets_delay_us(10000);
    SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_ACCEL_CONFIG, 0, 0b00010000);    //8G
    ets_delay_us(10000);

    //reading values without test enabled
    SPI_read_bytes(MPU_handle, 0, 0, 8, MPU6000_ACCEL_XOUT_H | MPU6000_READ_FLAG, 0, sensor_data, 14);
    accel_raw[0] = (sensor_data[0] << 8) | sensor_data[1];           //X
    accel_raw[1] = (sensor_data[2] << 8) | sensor_data[3];           //Y
    accel_raw[2] = (sensor_data[4] << 8) | sensor_data[5];           //Z
    gyro_raw[0] = (sensor_data[8] << 8) | sensor_data[9];            //X                  
    gyro_raw[1] = (sensor_data[10] << 8) | sensor_data[11];          //Y
    gyro_raw[2] = (sensor_data[12] << 8) | sensor_data[13];          //Z  

    ESP_LOGI(TAG_MPU6000,"Данные с акселерометра вне режима самодиагностики %d, %d, %d",accel_raw[0],accel_raw[1],accel_raw[2]);
    ESP_LOGI(TAG_MPU6000,"Данные с гироскопа вне режима самодиагностики %d, %d, %d",gyro_raw[0],gyro_raw[1],gyro_raw[2]);

    //enabling test bits
    SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_GYRO_CONFIG, 0, 0b11100000);     //+-250dps
    ets_delay_us(10000);
    SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_ACCEL_CONFIG, 0, 0b11110000);    //8G
    ets_delay_us(500000);
    //reading data with test enabled
    SPI_read_bytes(MPU_handle, 0, 0, 8, MPU6000_ACCEL_XOUT_H | MPU6000_READ_FLAG, 0, sensor_data, 14);
    accel_raw_test[0] = (sensor_data[0] << 8) | sensor_data[1];           //X
    accel_raw_test[1] = (sensor_data[2] << 8) | sensor_data[3];           //Y
    accel_raw_test[2] = (sensor_data[4] << 8) | sensor_data[5];           //Z
    gyro_raw_test[0] = (sensor_data[8] << 8) | sensor_data[9];            //X                  
    gyro_raw_test[1] = (sensor_data[10] << 8) | sensor_data[11];          //Y
    gyro_raw_test[2] = (sensor_data[12] << 8) | sensor_data[13];          //Z  

    ESP_LOGI(TAG_MPU6000,"Данные с акселерометра в режиме самодиагностики %d, %d, %d",accel_raw_test[0],accel_raw_test[1],accel_raw_test[2]);
    ESP_LOGI(TAG_MPU6000,"Данные с гироскопа в режиме самодиагностики %d, %d, %d",gyro_raw_test[0],gyro_raw_test[1],gyro_raw_test[2]);

    //calculating difference
    for (uint8_t i = 0; i < 3; i++) {
        str_accel[i] = accel_raw_test[i] - accel_raw[i]; //fabs as per https://github.com/natanaeljr/esp32-MPU-driver/blob/master/src/MPU.cpp????
        str_gyro[i] = gyro_raw_test[i] - gyro_raw[i];    //fabs????

        final_value_accel[i] = (str_accel[i] - factory_trim_calculated_values_accel[i]) / factory_trim_calculated_values_accel[i];
        final_value_gyro[i] =   (str_gyro[i] - factory_trim_calculated_values_gyro[i]) / factory_trim_calculated_values_gyro[i]; 

        if (fabs(final_value_accel[i]) > 0.14)
            {ESP_LOGE(TAG_MPU6000,"Самодиагностика акселерометра IMU[%d] не пройдена, вычисленное значение %0.2f за пределами диапазона [-0.14...0.14] range",i,final_value_accel[i]); err = ESP_FAIL;}
            else ESP_LOGI(TAG_MPU6000,"Самодиагностика акселерометра IMU[%d] успешно пройдена, вычисленное значение %0.2f",i,final_value_accel[i]); 

        if ((final_value_gyro[i] > 0.14) || (final_value_gyro[i] < -0.14))
            {ESP_LOGE(TAG_MPU6000,"Самодиагностика гироскопа IMU[%d] не пройдена, вычисленное значение %0.2f за пределами диапазона [-0.14...0.14]",i,final_value_gyro[i]); err = ESP_FAIL;}
            else ESP_LOGI(TAG_MPU6000,"Самодиагностика гироскопа IMU[%d] успешно пройдена, вычисленное значение %0.2f",i,final_value_gyro[i]);   
    }

    return err;
}     

esp_err_t MPU6000_init( spi_device_handle_t MPU_handle,
                        uint16_t sampling_freq_hz,
                        uint16_t integrated_lpf_value_hz,
                        uint16_t accel_full_scale_g,
                        uint16_t gyro_full_scale_dps) 

{
    uint8_t sampling_division_register = 0;
    uint8_t filter_register = 0;
    uint8_t accel_scale_register = 0;
    uint8_t gyro_scale_register = 0;

//формируем значения для записи в регистры на основании переданных в функцию параметров
//Для частоты сэмплирования
    switch (sampling_freq_hz) 
    {
        case 1000: sampling_division_register = 0; break;
        case 500: sampling_division_register = 1; break;
        case 333: sampling_division_register = 2; break;
        case 250: sampling_division_register = 3; break;
        case 200: sampling_division_register = 4; break;
        default: {ESP_LOGE(TAG_MPU6000,"Ошибка настройки частоты сэмплирования"); return (ESP_FAIL);}
    } 
//для значения встроенного фильтра    
    switch (integrated_lpf_value_hz) 
    {
        case 5: filter_register = 6; break;
        case 10: filter_register = 5; break;
        case 20: filter_register = 4; break;
        case 42: filter_register = 3; break;
        case 98: filter_register = 2; break;
        case 188: filter_register = 1; break;
        case 256: filter_register = 0; break;
        default: {ESP_LOGE(TAG_MPU6000,"Ошибка настройки LPF"); return (ESP_FAIL);}
    }
//для полной шкалы акселерометра    
    switch (accel_full_scale_g) 
    {
        case 2: accel_scale_register = 0; break;
        case 4: accel_scale_register = 8; break;
        case 8: accel_scale_register = 16; break;
        case 16: accel_scale_register = 24; break;
        default: {ESP_LOGE(TAG_MPU6000,"Ошибка настройки шкалы акелерометра"); return (ESP_FAIL);}
    }
//для полной шкалы гироскопа    
    switch (gyro_full_scale_dps) 
    {
        case 250: gyro_scale_register = 0; break;
        case 500: gyro_scale_register = 8; break;
        case 1000: gyro_scale_register = 16; break;
        case 2000: gyro_scale_register = 24; break;
        default: {ESP_LOGE(TAG_MPU6000,"Ошибка настройки шкалы гироскопа"); return (ESP_FAIL);}
    }
    esp_err_t err = ESP_OK;
    uint8_t reg_value = 0;
    uint8_t MPU6000_configuration_data[9][2] = {{MPU6000_PWR_MGMT_1,         0x80},         //сброс       
                                                {MPU6000_SIGNAL_PATH_RESET,  0x07},         //сброс                
                                                {MPU6000_PWR_MGMT_1,         0x01},         //внутренняя синхра с привязкой к гироскопу для лушей стабильности       
                                                {MPU6000_CONFIG,             filter_register},   //фильтр на 5Гц    
                                                {MPU6000_SMPLRT_DIV,         sampling_division_register},         //частота сэмплирования       
                                                {MPU6000_GYRO_CONFIG,        gyro_scale_register},         //гироскоп 250 град в сек макс               
                                                {MPU6000_ACCEL_CONFIG,       accel_scale_register},         //+-4G
                                                {MPU6000_USER_CTRL,          0b00010000},
                                                {MPU6000_INT_ENABLE,         0x01}};         

    for (uint8_t i = 0; i < 9; i++) {
        SPI_write_byte (MPU_handle, 0, 0, 8, MPU6000_configuration_data[i][0], 0, MPU6000_configuration_data[i][1]);
        ets_delay_us(10000);  
    }

    for (uint8_t i = 2; i < 9; i++) { //проверяем записанные значения
        reg_value = SPI_read_byte(MPU_handle, 0, 0, 8, MPU6000_configuration_data[i][0] | MPU6000_READ_FLAG, 0);
        if ( reg_value != MPU6000_configuration_data[i][1]) {
            err = ESP_FAIL;
            ESP_LOGE(TAG_MPU6000,"Ошибка конфигурирования MPU6000 в регистре %x, считано значение %d",MPU6000_configuration_data[i][0], reg_value);
        }
    }

    if (err == ESP_OK) ESP_LOGI(TAG_MPU6000,"MPU6000 настроен");
    else  ESP_LOGE(TAG_MPU6000,"Ошибка настройки MPU6000");

    return err;
}









