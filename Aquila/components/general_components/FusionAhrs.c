...
static inline FusionVector_t HalfGravity(const FusionAhrs_t *const ahrs);

static inline FusionVector_t HalfMagnetic(const FusionAhrs_t *const ahrs);

static inline FusionVector_t Feedback(const FusionVector_t sensor, const FusionVector_t reference);
...
const FusionAhrsSettings_t fusionAhrsDefaultSettings = {
...
void FusionAhrsUpdate(FusionAhrs_t *const ahrs, const FusionVector_t gyroscope, const FusionVector_t accelerometer, const FusionVector_t magnetometer, const float deltaTime) {
    ...
    if ((fabsf(gyroscope.axis.x) > ahrs->settings.gyroscopeRange) || (fabsf(gyroscope.axis.y) > ahrs->settings.gyroscopeRange) || (fabsf(gyroscope.axis.z) > ahrs->settings.gyroscopeRange)) {
        const FusionQuaternion_t quaternion = ahrs->quaternion;
        FusionAhrsReset(ahrs);
    ...
    const FusionVector_t halfGravity = HalfGravity(ahrs);

    // Calculate accelerometer feedback
    FusionVector_t halfAccelerometerFeedback = FUSION_VECTOR_ZERO;
    ...
    FusionVector_t halfMagnetometerFeedback = FUSION_VECTOR_ZERO;
    ahrs->magnetometerIgnored = true;
    if (FusionVectorIsZero(magnetometer) == false) {
        // Calculate direction of magnetic field indicated by algorithm
        const FusionVector_t halfMagnetic = HalfMagnetic(ahrs);
    ...
    const FusionVector_t halfGyroscope = FusionVectorScale(gyroscope, FusionDegreesToRadians(0.5f));

    // Apply feedback to gyroscope
    const FusionVector_t adjustedHalfGyroscope = FusionVectorAdd(halfGyroscope, FusionVectorScale(FusionVectorAdd(halfAccelerometerFeedback, halfMagnetometerFeedback), ahrs->rampedGain));
...
#define Q ahrs->quaternion.element
    switch (ahrs->settings.convention) {
        ...
        case FusionConventionEnu: {
            const FusionVector_t halfGravity = {
                .axis = {
                    .x = Q.x * Q.z - Q.w * Q.y,
                    .y = Q.y * Q.z + Q.w * Q.x,
...
#define Q ahrs->quaternion.element
    switch (ahrs->settings.convention) {
        case FusionConventionNwu: {
            const FusionVector_t halfMagnetic = {
                .axis = {
                    .x = Q.x * Q.y + Q.w * Q.z,
...
#undef Q
    ...
    const FusionVector_t magnetometer = {
        .axis = {
...
FusionAhrsInternalStates_t FusionAhrsGetInternalStates(const FusionAhrs_t *const ahrs) {
    const FusionAhrsInternalStates_t internalStates = {
...
#undef Q
    ...
    const FusionQuaternion_t rotation = {
        .element = {
