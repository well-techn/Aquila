...
uint8_t PMW3901_read_address(uint8_t address)
{
  ...
  ets_delay_us(1);
  SPI_write_byte (PMW3901, 0, 0, 0, 0, 0, address);
  ets_delay_us(35);
  value = SPI_read_byte (PMW3901,  0, 0, 0, 0, 0);
  ets_delay_us(1);
...
void PMW3901_read_motion_burst(uint8_t* buffer)
{
  gpio_set_level(GPIO_CS_PMW3901,0);
  ets_delay_us(1);
  SPI_write_byte (PMW3901, 0, 0, 0, 0, 0, PMW3901_MOTION_BURST);
  ets_delay_us(35);
  SPI_read_bytes (PMW3901, 0, 0, 0, 0, 0, buffer, 12);
  ets_delay_us(1);
  gpio_set_level(GPIO_CS_PMW3901,1);
  ets_delay_us(1);
}
