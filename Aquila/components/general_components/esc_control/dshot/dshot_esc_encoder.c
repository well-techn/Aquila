...
// static esp_err_t dshot_fast_del(rmt_encoder_t *encoder)
// {
//     // Получаем указатель на нашу структуру из базового интерфейса
//     dshot_fast_encoder_t *fast_encoder = __containerof(encoder, dshot_fast_encoder_t, base);
    
//     // 1. Удаляем внутренний copy_encoder (важно, это системный ресурс)
//     if (fast_encoder->copy_encoder) {
//         rmt_del_encoder(fast_encoder->copy_encoder);
//     }
    
//     // 2. Освобождаем память самой структуры
//     free(fast_encoder);
...
// static esp_err_t IRAM_ATTR dshot_fast_encoder_reset(rmt_encoder_t *encoder)
// {
//     dshot_fast_encoder_t *fast_encoder = __containerof(encoder, dshot_fast_encoder_t, base);
//     // Нам нужно сбросить только внутренний copy_encoder
//     return rmt_encoder_reset(fast_encoder->copy_encoder);
// }


// esp_err_t rmt_new_fast_dshot_esc_encoder(const dshot_esc_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder)
// {
//     dshot_fast_encoder_t *fast_encoder = NULL;
//     // 1. Выделяем память под структуру энкодера
//     fast_encoder = rmt_alloc_encoder_mem(sizeof(dshot_fast_encoder_t));
//     if (!fast_encoder) return ESP_ERR_NO_MEM;

//     // 2. Назначаем функции управления
//     fast_encoder->base.encode = rmt_encode_fast_dshot_esc; // Наша быстрая функция
//     fast_encoder->base.del = dshot_fast_del; // Наша кастомная функция
//     fast_encoder->base.reset = dshot_fast_encoder_reset; // Наша кастомная функция

//     // 3. Рассчитываем тайминги (тики) для битов 0 и 1 (аналогично стандарту)
//     float period_ticks = (float)config->resolution / config->baud_rate;
//     uint32_t t1h = (uint32_t)(period_ticks * 0.75); // DShot: 75% времени - High для "1"
//     uint32_t t1l = (uint32_t)(period_ticks - t1h);
//     uint32_t t0h = (uint32_t)(period_ticks * 0.375); // DShot: 37.5% времени - High для "0"
//     uint32_t t0l = (uint32_t)(period_ticks - t0h);

//     // Заполняем символы "1" и "0"
//     fast_encoder->sym_one = (rmt_symbol_word_t){.level0 = 1, .duration0 = t1h, .level1 = 0, .duration1 = t1l};
//     fast_encoder->sym_zero = (rmt_symbol_word_t){.level0 = 1, .duration0 = t0h, .level1 = 0, .duration1 = t0l};

//     // 4. Рассчитываем символ паузы (post_delay)
//     uint32_t delay_ticks = (uint32_t)(config->resolution / 1e6 * config->post_delay_us);
//     fast_encoder->sym_delay = (rmt_symbol_word_t){.level0 = 0, .duration0 = delay_ticks, .level1 = 0, .duration1 = 0};

//     // 5. Создаем внутренний copy_encoder для финального сброса данных в RMT
//     rmt_copy_encoder_config_t copy_conf = {};
//     rmt_new_copy_encoder(&copy_conf, &fast_encoder->copy_encoder);

//     *ret_encoder = &fast_encoder->base;
//     return ESP_OK;
// }

// //непосредственно процесс кодирования
// size_t rmt_encode_fast_dshot_esc(rmt_encoder_t *encoder, rmt_channel_handle_t channel,
//                                           const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
// {
//     dshot_fast_encoder_t *fast_encoder = __containerof(encoder, dshot_fast_encoder_t, base);
//     const dshot_esc_throttle_t *data = (const dshot_esc_throttle_t *)primary_data;
    
//     // 1. Быстрый расчет CRC
//     uint16_t throttle = data->throttle;
//     uint16_t telemetry = data->telemetry_req ? 1 : 0;
//     uint16_t frame = (throttle << 5) | (telemetry << 4);
//     uint16_t crc = ((frame ^ (frame >> 4) ^ (frame >> 8)) & 0x0F);
//     frame |= crc; 

//     // 2. Готовим массив символов прямо на стеке (очень быстро)
//     rmt_symbol_word_t symbols[17];
//     for (int i = 0; i < 16; i++) {
//         symbols[i] = (frame & (0x8000 >> i)) ? fast_encoder->sym_one : fast_encoder->sym_zero;
//     }
//     symbols[16] = fast_encoder->sym_delay;

//     // 3. Сбрасываем весь массив в RMT за один вызов базового копировщика
//     // Это и есть секрет скорости: один вызов вместо 17-ти.
//     rmt_encoder_handle_t copy_enc = fast_encoder->copy_encoder;
//     return copy_enc->encode(copy_enc, channel, symbols, sizeof(symbols), ret_state);
// }
